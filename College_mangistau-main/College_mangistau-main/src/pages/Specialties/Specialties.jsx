            ...
            <Header />
                ...
                <div className={styles.header}>
                    <div className={styles.headerInner}>
                        <h1 className={styles.title}>{t('specialties.title','Специальности')}</h1>
                        <p className={styles.subtitle}>{t('specialties.detail.subtitle','Информация о специальностях')}</p>
                    </div>
                </div>
            ...
            <div className={styles.grid}>
                {specialties.map(s => (
                    <article key={s.id} className={styles.card}>
                        ...
                        <Link to={`/specialties/${s.id}`} className={styles.cardMore}>{t('specialties.more_btn','Подробнее')}</Link>
                    </article>
                ))}
            </div>
        </section>
